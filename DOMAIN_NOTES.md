...
- **Specific Failure Mode (Coupling)**: If a `ComplianceRuleFailed` event were forced to write to the `LoanApplication` stream, it would require an optimistic lock on that stream. If four other agents are simultaneously trying to write their progress to the same `LoanApplication` stream, the compliance agent would suffer constant write contention and serial delays, even though its rule failure is logically independent of the other agents' internal nodes.
