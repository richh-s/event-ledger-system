...
- **Upcasting Trace**:
  ...
  # }
  ```
- **Inference Strategy**: `model_version` is inferred from deployment logs; `confidence_score` is set to `null` to avoid fabricating historical statistics.

### 6.2 Cryptographic Hash Chain Demonstration
Every event is linked to its predecessor via a SHA-256 hash ($H_n = Hash(H_{n-1} + EventContent_n)$).
...
**Verification Query**: `ledger://applications/APP-7781/compliance` returns a complete, cryptographically verified trace showing all 7 events with matching correlation IDs and no gaps.
